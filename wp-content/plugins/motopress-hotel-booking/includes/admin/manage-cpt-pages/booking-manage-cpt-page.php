...
class BookingManageCPTPage extends ManageCPTPage {
		...
		switch ( $column ) {
            ...
            case 'room_type':
                ...
                foreach ($reservedRooms as $reservedRoom) {
                    ...
                    if ($roomTypeId == 0 || array_key_exists($roomTypeId, $roomTypeLinks)) {
                        continue;
                    }

                    $roomType = MPHB()->getRoomTypeRepository()->findById($roomTypeId);
                    $roomType = MPHB()->translation()->translateRoomType($roomType);

                    if (is_null($roomType)) {
                        continue;
                    }

                    $editUrl = get_edit_post_link($roomType->getOriginalId());

                    $roomTypeLinks[$roomTypeId] = '<a href="' . esc_url($editUrl) . '">' . esc_html($roomType->getTitle()) . '</a>';
                }

                echo !empty($roomTypeLinks) ? implode(', ', $roomTypeLinks) : self::EMPTY_VALUE_PLACEHOLDER;
    ...
    public function addAccommodationsFilter()
    {
        ...
        echo '<option value="0" ' . selected($selectedId, 0, false) . '>', __('All accommodation types', 'motopress-hotel-booking'), '</option>';
