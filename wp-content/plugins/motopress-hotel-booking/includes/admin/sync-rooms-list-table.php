...
class SyncRoomsListTable extends \WP_List_Table
{
	...
	protected function filter_items($items)
	{
        ...
        foreach ($items as $queueId => $item) {
            ...
            switch ($item['status']) {
                case Queue::STATUS_WAIT:        $statusText = __('Waiting', 'motopress-hotel-booking');    break;
                case Queue::STATUS_IN_PROGRESS: $statusText = __('Processing', 'motopress-hotel-booking'); break;
                case Queue::STATUS_DONE:        $statusText = __('Done', 'motopress-hotel-booking');       break;

                default: $statusText = ucfirst(str_replace('-', ' ', $item['status'])); break;
            }
	...
	public function column_title($item)
	{
		...
		$output = '<h4><a href="' . esc_url($viewUrl) . '">' . $item['title'] . '</a></h4>';
    ...
    public function column_status($item)
    {
        ...
        return '<span class="' . $class . '">' . $item['status-text'] . '</span>';
    }
