...
class BookingsCalendar {
	...
	private function renderPseudoCell( $roomId, $date ){
		...
		if ( $dateDetails['is_locked'] ) {
			...
			if ( $dateDetails['is_check_in'] ) {
				$secondPartContent = '<a class="mphb-link-to-booking" href="' . $dateDetails['booking_edit_link'] . '" data-booking-id="' . $dateDetails['booking_id'] . '" data-booking-status="' . $dateDetails['booking_status'] . '">' . $dateDetails['booking_id'] . '</a>';
			} else {
